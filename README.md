...
Application tables live in `supabase/migrations/`. `isolation_probes` and `isolation_catalog` are the isolation fixtures; product tables are still added by the slice that needs them.
